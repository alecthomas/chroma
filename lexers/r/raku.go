...
var RakuRules = Rules{
	...
	"pod-blocks": {
		...
		{
			`(?<=^ *)( *=)(alias)( +)(\w[\w'-]*)([^\n]*\n)`,
			ByGroups(Keyword, Keyword, StringDoc, Keyword, StringDoc),
			nil,
		},
		// =encoding
		{
			`(?<=^ *)( *=)(encoding)( +)([^\n]+)`,
			ByGroups(Keyword, Keyword, StringDoc, Name),
			nil,
		},
		// =para ...
		{
			`(?<=^ *)( *=)(para|table|pod)((?<!\n\s*)[^\n]*\n)`,
			ByGroups(Keyword, Keyword, EmitterFunc(podConfig)),
			Push("pod-paragraph"),
		},
		// =head1 ...
		{
			`(?<=^ *)( *=)(head\d+)( *)(#?)`,
			ByGroups(Keyword, Keyword, GenericHeading, Keyword),
			Push("pod-single-heading"),
		},
		// =item ...
		{
			`(?<=^ *)( *=)(item\d*|comment|data|[A-Z]+)( *)(#?)`,
			ByGroups(Keyword, Keyword, StringDoc, Keyword),
			Push("pod-single"),
		},
		{
			`(?<=^ *)( *=)(finish)([^\n]*)`,
		...
		{
			`(?<=^ *)( *=)(\w[\w'-]*)( *)(#?)`,
			ByGroups(Keyword, Name, StringDoc, Keyword),
			Push("pod-single"),
		},
		// = podconfig
		{
...
var keywordsPattern = Words(`(?<!['\w:-])`, `(?!['\w:-])`, sortWords(keywords)...)

var wordOperators = []string{
...
var wordOperatorsPattern = Words(`(?<=^|\b|\s)`, `(?=$|\b|\s)`, sortWords(wordOperators)...)
...
var operatorsPattern = Words(``, ``, sortWords(operators)...)
...
var builtinTypesPattern = Words(`(?<!['\w:-])`, `(?::[_UD])?(?!['\w:-])`, sortWords(builtinTypes)...)
...
var builtinRoutinesPattern = Words(`(?<!['\w:-])`, `(?!['\w-])`, sortWords(builtinRoutines)...)
...
var bracketsPattern = `[` + regexp.QuoteMeta(joinRuneMap(brackets)) + `]`
...
func bracketsFinder(tokenClass RakuToken) MutatorFunc {
...
func podCode(groups []string, lexer Lexer) Iterator {
	...
	if sublexer != nil {
		...
		if codeIteratorError != nil {
			...
			iterators = append(iterators, codeIterator)
		}
	} else {
		iterators = append(iterators, Literator(tokens[4]))
	}
